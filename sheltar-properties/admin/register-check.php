...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($phone_err) && empty($password_err) && empty($confirm_password_err) && empty($firstname_err) && empty($lastname_err) && empty($email_err)) {
        ...
        $query = "INSERT INTO users (first_name, last_name, email, phone, password, code, code_expiry, registerDate ) VALUES(?, ?, ?, ?, ?, ?, ?, NOW())";

        if ($stmt = mysqli_prepare($conn, $query)) {
            mysqli_stmt_bind_param($stmt, 'sssssis', $param_firstname, $param_lastname, $param_email, $param_phone, $param_password, $param_code, $param_code_expiry);
